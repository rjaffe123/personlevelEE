...
#' @param ci Value of confidence interval calculated and displayed, or all three on one graph
#'
#' @return
#' @export
#'
#' @examples
define_fieller <- function(cost, effect, ci = c(95, 75, 50, "all")) {
  ...
  ellipse_data <- ellipse_data %>% mutate(cat("part1_c", ci, sep="") = sqrt(-2*log(1-ci))*se_dc,
                                          cat("part1_c", ci, sep="") = sqrt(-2*log(1-ci))*se_de)

  ellipse_data <- ellipse_data %>% mutate(part2_c = cos(theta-acos(corr)/2), part2_e = cos(theta+acos(corr)/2))
  ellipse_data <- ellipse_data %>% mutate(cat("delta_c", ci, sep="")= part1_c95*part2_c + dc,
                                          cat("delta_c", ci, sep="") = part1_e95*part2_e + de)
  }
  else {
  ##95
  ellipse_data <- ellipse_data %>% mutate(theta = pi*2*(id-1)/(n0 + n1 - 1))
...
plot.define_fieller <- function(object){
    ...
    geom_point(aes(x = de, y = dc), color = "black") + theme_stata() +
    geom_vline(xintercept = 0, color ="grey") +
    geom_hline(yintercept = 0, color = "grey") +
    xlab(expression(Delta*"E"))+ylab(expression(Delta*"C"))+
    scale_colour_manual(name = 'Confidence Interval',
                        values =c('red'='red','blue'='blue','green'='green'), labels = c('95%','75%','50%')) +labs(subtitle = "Basic Regression")+theme(axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1))
}
