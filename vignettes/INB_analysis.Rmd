...
covariates_list <- define_covariates(covariates = list(data_example$age,
                                     ...
                                     id = data_example$id)
## run the model
nb_model <- run_INB_model(nb_values = nb_value, covariates = covariates_list)
...
If we define more than one value for lambda, we can create a cost effectivness acceptability curve based on the regression results. We can plot $1-p/2$ against the different values of $\lambda$, where p is the p-value associated with the coefficient on the treatment dummy ($X_{trt}$).
